...
class MyCDCCallbacks : public CDCCallbacks {
  ...
  bool onConnect(bool dtr, bool rts) {
    ...
    return true;  // allow to persist reset, when Arduino IDE is trying to enter
                  // bootloader mode
  }
      ...
      for (int i = 0; i < bytesRead; i++) {
        if (data[i] == 0x0a) {
          if (currentLine.indexOf("vid") >= 0) {
            ...
            String vidString = currentLine.substring(
          ...
          if (currentLine.indexOf("pid") >= 0 && pidEn) {
            ...
            String pidString = currentLine.substring(
                currentLine.indexOf(":") + 1, currentLine.length());
          ...
          if (currentLine.indexOf("network") >= 0) {
            networkName = currentLine.substring(currentLine.indexOf(":") + 1,
                                                currentLine.length());
            // networkName.trim();
          }

          if (currentLine.indexOf("password") >= 0) {
            netPassword = currentLine.substring(currentLine.indexOf(":") + 1,
                                                currentLine.length());
            // netPassword.trim();
          }
