...
void processDuckyScript(String ducky, SH1106Wire* display,
  ...
  if (tCommand.equals("REM")) {
    ...
    display->drawString(
        3, 22, (String)ducky.substring(ducky.indexOf(' ') + 1, ducky.length()));
    display->drawXbm(0, 0, 128, 64, cat_with_reload_spinner_image_bits);
    display->display();
    delay(ducky.substring(ducky.indexOf(' ') + 1, ducky.length())
              .toInt());  // delay in MS
    Serial.println("Delayed!");
  } else if (tCommand.equals("DEFAULT_DELAY") or
             tCommand.equals("DEFAULTDELAY")) {
    display->drawString(3, 12, "DEFAULT");
    display->drawString(3, 22, "DELAY:");
    display->drawString(
        3, 32, (String)ducky.substring(ducky.indexOf(' ') + 1, ducky.length()));
    display->drawXbm(0, 0, 128, 64, cat_with_reload_spinner_image_bits);
    display->display();
    defaultDelay =
        ducky.substring(ducky.indexOf(' ') + 1, ducky.length()).toInt();
  } else if (tCommand.equals("LED")) {
    display->drawString(3, 12, "COLOR:");
    display->drawString(3, 22,
                        (String)ducky.substring(
                            ducky.indexOf(' ') + 1,
                            ducky.length()));  // accept single color parameter
    display->drawXbm(0, 0, 128, 64, cat_with_reload_spinner_image_bits);
    display->display();
    String color =
        (String)ducky.substring(ducky.indexOf(' ') + 1, ducky.length());
    color.toUpperCase();
    ...
    if (String(ducky.substring(ducky.indexOf(' ') + 1, ducky.length()))
            .length() > 11) {
      display->drawString(
          3, 22,
          String(ducky.substring(ducky.indexOf(' ') + 1, ducky.length()))
    ...
    while (currentTokenLeftIndex < ducky.length()) {
      ...
      currentToken =
          ducky.substring(currentTokenLeftIndex, currentTokenRightIndex);
